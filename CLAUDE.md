...
- **드래그 방향 초기 emit race 수정 (v0.1.37)**: 사용자 보고 — "드래그해서 왼쪽으로 이동할 때 왼쪽으로 달리는 모션이 보여야하는데, 이상한 모션이 보이고 있어. 오른쪽으로 이동할 때도 오른쪽으로 달리는 모션이 보이게 수정해". v0.1.34 에서 도입한 `companion:facing` emit 이 `cmd_begin_drag` 진입 시점에 항상 `{dragging: true, facing_left: false}` 를 **하드코딩** 으로 발사하던 게 원인. 좌향 드래그 타임라인: t=10ms 프런트가 `cmd_begin_drag` 호출(이미 cursor 6px+ 좌측 이동) → t=15ms Rust 초기 emit `facing_left=false` → t=20ms 프런트가 RUNNING 시작하지만 우향(flip 없음) → t=48ms 루프 tick 1 이 dx<-4 감지해 `facing_left=true` emit → t=55ms 프런트가 비로소 scaleX(-1) 적용. 결과: **약 35ms 동안 "좌로 이동하는데 우로 달리는" 시각 충돌** → 사용자가 "이상한 모션" 으로 인지. 우향 드래그는 초기값이 우연히 맞아서 정상이라 안 보였음. **수정**: (1) `BeginDragInput` 에 `initial_facing_left: bool` 필드 추가 (`#[serde(rename_all = "camelCase")]` 유지). (2) `cmd_begin_drag` 가 `input.initial_facing_left` 로 초기 emit, 동일 값을 `run_drag_cursor_loop(...)` 새 파라미터로 전달. (3) 루프가 `last_facing_left = initial_facing_left` 로 시드 — 초기 emit 값과 일치시켜 첫 hysteresis 진입 시 같은 값을 중복 emit 하지 않도록. (4) 프런트 `handlePointerMove` 가 이미 임계값 검사를 위해 계산하는 `dx = event.screenX - state.startScreenX` 의 부호로 `initialFacingLeft: dx < 0` 를 IPC payload 에 추가 — 별도 cursor 폴링 불필요. **불변 규칙**: 사용자 입력 핫패스의 시각 응답을 백엔드가 "기본값" 으로 추론하지 말 것 — 프런트가 이벤트 핸들러 시점에 이미 알고 있는 정보(여기선 cursor delta 부호)는 명시 전달해야 IPC round-trip + loop 검출 지연으로 인한 첫 프레임 mismatch 가 사라진다. 같은 패턴이 적용 가능한 경로: 드래그 시작 외에 swipe / scroll / hold 등 "임계값 진입 직후 첫 emit" 이 필요한 모든 입력 IPC. 백엔드 기본값(false / 0 / None) 으로 시작하면 사용자가 정확히 그 값과 반대 방향으로 입력할 때 visible flash 가 발생한다.
